 ...
 * 
 */
package esnagofer.msx.ide.emulator.core.application.javafx;
...
public class JavafxKeyboard implements Keyboard {
	...
	private Optional<RowBit> keyMatrixRowCol(KeyCode keyCode) {
		RowBit rowBit = null;
		switch (keyCode) {
		// 6.0 to 6.7
	...
	private void processKeyEvent(KeyEvent e, boolean state) {
		...
		if (!success) {
			Optional<RowBit> optionalRowBit = keyMatrixRowCol(e.getCode());
