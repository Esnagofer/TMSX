package msx.emulator.core.domain.model.hardware.tms9918a;

import msx.emulator.core.domain.model.hardware.memory.Memory;
import msx.emulator.core.domain.model.hardware.screen.Color;
import msx.emulator.core.domain.model.hardware.screen.Screen;
import msx.emulator.core.domain.model.lib.Tools;
...
public class TMS9918A {
	...
	public static final int IMG_SCALE = 2, VDP_WIDTH = 256, VDP_HEIGHT = 192, MODE0_OFFSET = 8;
	 ...
	 *            the new status FS 4
	 */
	public void setStatusFS4(boolean v) {
		setStatusBit(4, v);
	}
	 ...
	 *            the new status FS 3
	 */
	public void setStatusFS3(boolean v) {
		setStatusBit(3, v);
	}
	 ...
	 *            the new status FS 2
	 */
	public void setStatusFS2(boolean v) {
		setStatusBit(2, v);
	}
	 ...
	 *            the new status FS 1
	 */
	public void setStatusFS1(boolean v) {
		setStatusBit(1, v);
	}
	 ...
	 *            the new status FS 0
	 */
	public void setStatusFS0(boolean v) {
		setStatusBit(0, v);
	}
	...
	public final void increaseReadWriteAddr() {
		...
		if ((readWriteAddr & 0xFFFF) == 0x3FFF)
			readWriteAddr = 0;
	}
	 ...
	 */
	/*
	 * Return value of status bit. The side-effects of doing this is that the INT
	 * and C flags are reset, and that the second byte flag is reset, which affects
	 * the writeRegister behaviour.
	 */
...
//		px <<= IMG_SCALE - 1;
//		py <<= IMG_SCALE - 1;
//		if (px + 1 >= VDP_WIDTH * IMG_SCALE || py + 1 >= VDP_HEIGHT * IMG_SCALE || px < 0 || py < 0)
//			return;
	...
	public void drawSprites() {
		...
		for (int x = 0; x < VDP_WIDTH; x++)
			for (int y = 0; y < VDP_HEIGHT; y++)
				spritePriorityMatrix[x][y] = false;

		// Clear collision array
		for (int x = 0; x < VDP_WIDTH; x++)
			for (int y = 0; y < VDP_HEIGHT; y++)
				spriteCollisionMatrix[x][y] = false;

		// For each sprite ...
		for (int i = 0; i < 32; i++) {

			// Break if 0xD0 encountered
			if ((getSpriteY(i) & 0xff) == 0xD0)
				break;
			...
			for (int yPos = sy; yPos < sy + (siFlag ? 16 : 8) && yPos < VDP_HEIGHT; yPos++)
				spriteLineCount[yPos]++;

			// If sprite is transparent then skip
			if ((colour & 0x0f) == 0)
				continue;

			// Draw sprite (qx/qy range over quadrants for 16x16 mode. In 8x8 mode, qx = qy
			// = 0)
				...
				for (int qy = 0; qy < (siFlag ? 2 : 1); qy++) {
					...
					for (int y = 0; y < 8; y++) {
						for (int x = 0; x < 8; x++) {
							...
							if (spriteCollisionMatrix[xPos][yPos])
								setStatusC(true);
							spriteCollisionMatrix[xPos][yPos] = true;

							// Do we need to fill this pixel?
							boolean fill = (mem
									.rdByte((short) (patternTableAddr + (8 * (patternIdx + quadrantNumber) + y)))
									& (1 << (7 - x))) != 0;
							if (!fill)
								continue;

							// Are there already 4 sprites drawn on the current line? I so, mark 5th sprite
							// flag and status register, and skip
							...
							// We iterate from high priority to low priority. Skip if a higher priority
							// sprite was already drawn here.
							if (spritePriorityMatrix[xPos][yPos])
								continue;
